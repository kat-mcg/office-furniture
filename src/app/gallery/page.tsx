...
export default function GalleryPage() {
  ...
  const uniqueCategories = Array.from(new Set(
    items.map((i) => i.category?.name).filter(Boolean) as string[]
  )).sort();
    ...
    <div>
      ...
      {filteredItems.length === 0 && items.length === 0 ? (
        ...
        <div className="card text-center py-12 px-6">
          ...
          <button onClick={() => setFilterCategory("")} className="text-sm text-indigo-600 hover:text-indigo-700 mt-2">
            Clear filter
          </button>
        </div>
      ) : (
              ...
              <div key={item.id} className="card overflow-hidden flex flex-col">
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    ...
                    {fit !== "unknown" && (
                      <span className={`badge ${fitColor(fit)}`}>
                        {fitLabel(fit)}
                      </span>
                    )}
                    {item.inCart && (
